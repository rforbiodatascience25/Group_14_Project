...
![](images/clipboard-541288159.png)
...
-   SST(A): Sea Surface Temperature (Anomaly)
    ...
    -   The SST is the measured sea surface temperature in Kelvin for the given observation. SSTA is the degree Kelvin difference compared to the normal temperature at that site (typically based on a week or month). The temperatures were derived from the *Coral Reef Temperature Anomaly Database* by the article authors.
...
During the project, we have followed the style of the "Data Science Cycle" as seen below. The work has primarily been focused on tidying as well as visualisation.![](images/clipboard-1447661144.png)
...
We make a plot based on the different seas to see if any oceans stands out.
...
We conclude that the East pacific has too few data points for further analysis when comparing to the other seas. Hence, we filter it out.
...
We look at the temperatures in the different oceans too get insight into the dataset:
...
The reason for the variations in temperature is that the samples are taken in different depths, sites and timepoints \[1\].
...
ggplot(data = reef_clean,
       mapping = aes(x = Temperature_Celsius,
                     y = Depth)) +
  geom_point(color= "#0D0887", alpha = (0.5)) +
  labs(x = "Temperature Celsius", y = "Depth [m]", title = "Temperatures based on depth of the sampling", fill = "Ocean", color = "Ocean") +
  theme_minimal(base_family = "Avenir",
                base_size = 12) +
  theme(legend.position = "bottom")+
  facet_wrap(~ Ocean)
...
The depths will not be taken into further account and doesn't seem to influence the temperature in the seas.
...
The average bleaching seems higher for latitudes from -5 and further south and +5 and further north. Also, it seems like there is more average bleaching at higher latitudes. A hypothesis can be that it is due to higher species diversity at lower latitudes, resulting in a higher robustness against temperature variations \[1\].
...
Coral bleaching is a global issue and requires samples from around the world, resulting in enormous datasets. To evaluate some of the findings from these many data points from 2002-2017 R has been a good tool. We show some general information about the data, and can conclude that there is a tendency that coral bleaching is increasing in the Pacific. Similar analysis could be made on the other Oceans but there should be an attention to if the ocean has enough data points.
