...
The consequences of coral bleaching affect the climate in terms of ecosystem damage due to loss of species and thereby biodiversity, but it also has major social and economic consequences for people who depend on reefs for food and tourism. The species loss can also mean indirect consequences in research e.g., loss of molecules that could've been discovered and used pharmaceutically or elsewhere.
...
We want to remove all the observations where Ocean has NA values and we can see in the following that 57 observations has NA values. For this we can use filter to remove them from the dataset.
...
Wee want to check how many observations we have for each year.
...
We see that three years (1998, 2000 and 2001) does not contain many observations so these we want to filter out as well.
...
We see that the East Pacific only has few data points. We count them:
...
Making a column with the temperatures in degrees celsius:
...
We investigate the temperature development over time:
...
We investigate the latitude and temperature connection:
...
We note that there's a great variation in the measured temperatures at the same latitudes. We investigate the depths of the sampling affect on the temperature:
...
We combine ggplot an a pipeline to only see Pacific Ocean data with latitudes and average bleaching:
